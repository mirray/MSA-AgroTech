...
Сотрудники фермы могут получать информацию о состоянии инфраструктуры фермы и поголовья скота через мобильное или веб приложение. При этом должна осуществляться отправка метрик из АТСК в общий аналитический модуль компании.

Значительное влияние на архитектуру могут дать такие ограничения как +R6 - нестабильный Wi-Fi, при этом применяются серьезные требования к времени реакции видеонаблюдения (P3, P4). Таким образом, систему можно построить по архитектуре сервер-агенты, при этом агенты синхронизируются с сервером в случае появления интернета. 
...
В случае если ферма использует другие методы идентификации и переход на RFID-метки будет сложным или затратным, возможно использовать альтернативный вариант. Этот вариант предполагает идентификацию свиней по видео.
...
Ограничения альтернативного подхода:
 - Идентификация свиней через видео может быть достаточно сложной, так как особи похожи между собой
 - Дополнительные издержки - для визуальной идентификации свиней придется использовать камеры, которые могут снимать в хорошем качестве, что не всегда может быть на ферме, особенно если там уже построена своя инфраструктура видеонаблюдения
